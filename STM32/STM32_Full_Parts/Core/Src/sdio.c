...
void cliShowSdInfo(void *arg)
{
    ...
    if (State == HAL_SD_CARD_TRANSFER) {
        ...
        tftprintf("Car capacity %ld",
               ((uint32_t) hsd.SdCard.BlockSize *
                hsd.SdCard.BlockNbr));
        ...
        tftprintf("Get SD card info fail.");
    }
}
    ...
    if (State == HAL_SD_CARD_TRANSFER) {
        if (f_opendir(&dir, "") == FR_OK) {
            ...
            if (!finfo.fname[0])
                break;
            tftprintf
                ("%-20s%-10d%u-%02u-%02u, %02u:%02u",
                 finfo.fname, finfo.fsize, (finfo.fdate >> 9) + 1980,
                 finfo.fdate >> 5 & 15, finfo.fdate & 31,
                 finfo.ftime >> 11, finfo.ftime >> 5 & 63);
            }
        } else {
          tftprintf("Get SD card dir fail.");
        }
    } else {
        tftprintf("Get SD card info fail.");
    }
